...
| `listPeople` | `GET /permissions`, ≥viewer to read, owner to change | ✅ | **added to the backend**: reading the list was gated at owner, so everyone else opened "People with access" and saw themselves alone. The answer carries `can_manage`, and the modal renders read-only for anybody who does not have it. Rows now name their `principal` — `user` or `group` — and a group row carries `group_id`, `group_name` and `member_count` instead of the user fields; `listPeople` namespaces the group's id as `g:<id>` (the two id spaces are separate) and remembers the principal beside the grant id, because `PATCH` and `DELETE` address a group grant with `?principal=group` |
| `addPerson` / `addGroup`, `searchGroups` | `POST /permissions/invite {path, level, is_dir, email\|group_id}`; `GET /permissions/groups?q=&limit=` | ✅ | **added to the backend**. Three things changed on this side. `is_dir` is the NODE's kind now — it was hard-coded `true`, so every grant on a file was recorded against a folder that does not exist. A group is granted by id, from a debounced type-ahead in the invite row. And the invite answers with a `mode`: `granted`, `created`, or — for an address with no account behind it — `shared`, meaning filex minted a PUBLIC LINK instead, which the modal shows with its URL and adds nobody to the list. `owner` is a grantable level for a caller who may manage access. A drive with RBAC off answers 409 and the modal says who can switch it on |
| role permissions | `permissions` on `GET /api/files/capabilities`; 403 `{"code":"ROLE_FORBIDDEN","op":…}` on a refusal | ✅ | **added to the backend**: the driver flags say what the INSTALLATION can do, and these say what this caller's ROLE may do — a second, independent gate over the same menus (`files.upload`, `.mkdir`, `.rename`, `.move`, `.copy`, `.delete`, `.purge`, `.restore`, `.download`, `.share`, `.grant`, `.tags`, `.star`). Both have to say yes; the entry is disabled with "Your role may not do this" when the role is the reason and "Not available on this server" when the server is. ⚠ A server that does not send the field is read as granting ALL of them — it has no such rule to state, and reading silence as refusal would empty every menu the day the app ships ahead of the backend. Drag-and-drop is inert without `files.move`, and a `ROLE_FORBIDDEN` that slips through anyway becomes the same sentence as a toast |
| `listStorages` quota | `GET /api/files/quota/me` | ✅ | three ceilings, not one: bytes held, file count (`used_files` / `quota_files`, 0 = unlimited) and bytes uploaded inside a rolling window (`upload_used_bytes` / `upload_quota_bytes` / `upload_window_hours`). The sidebar's bar still draws the first and a caption states the other two where they exist; settings repeats all three read-only. The refusals carry their own figures — 413 `FILE_LIMIT_EXCEEDED` with `limit`/`used`, and **429** `UPLOAD_RATE_LIMITED` with `retry_after_seconds` (`Retry-After` as the fallback), which the upload tray waits out and re-queues rather than reporting as a failure |
...
| `listFolder` with a `ListingFilter` | `q=index` takes `ext`, `modified_after`, `size_min`, `size_max`, `owner_id` and `name` (a case-insensitive substring of the file name), inside the query, and answers `total` | ✅ | **the server's work now.** `total` is the folder's UNFILTERED entry count, so a narrowed answer can be told from an empty folder. Folders are kept by `name`, `modified_after` and `owner_id` and dropped by `ext` and the size band — the same rule the flat listings apply. The cold-cache driver read applies the same predicate in Go; `action=subfolders` takes no filter |
| `listShared` with a `ListingFilter` | `shared-with-me` takes the same six facets, applied before the page is cut, so `total` counts the filtered set | ✅ | **the server's work now.** A row synthesised from a grant the indexer never walked has no size, date or owner for a chip to test: it is listed when nothing is asked and dropped by any chip |
...
|---|---|---|
| Filters on the folder listing and on shared-with-me | both endpoints now take `ext`, `modified_after`, `size_min`, `size_max`, `owner_id`, `name` and answer `total` (see the two rows above) | `HttpRepository` may keep `data/listingFilter.ts` for the mock, but the server's answer is already narrowed; a client that still filters locally should do so only when `files.length === total`. The app's rule: a folder whose `total` is under 1000 is held whole and sieved in memory (`stores/files.ts`, `CLIENT_FILTER_MAX`), so a chip costs no request there; from 1000 up, and on every flat listing, each chip change is a request with the facets |
...
- **Owner**: nodes DO have one — `nodes.owner_id`, written by quota accounting
  ...
  for everything. The GROUP owner that was missing here now exists: filex has
  groups, and a drive reached through one says so. A node a storage SYNC found
  still has no owner at all and falls back to the caller, which is the honest
  reading of "everything you can see, you can see".
- **The group owner is the GROUP, and the drive where there is none.** filex has
  groups now: they hold grants like accounts do, and `GET /api/files/storages`
  carries `via_groups` per drive — the groups through which THIS caller reaches
  it. So the Owner column names the first of them ("Design") rather than the
  mount ("Marketing"), because a drive reached through a team is the team's; a
  drive with no group behind it keeps naming the drive, which is what `shared`
  per drive has always been for. `shared` is still derived from the test
  `shared-with-me` already makes: grants are loaded only for a non-admin on an
  RBAC-enabled storage, so holding any is exactly the condition. The column
  never names a person on such a drive — that was noise, and still is.
...
- **N-20 — upload conflicts are decided on the server (done).** `if_exists`
  (`replace` | `fail`) on `upload/begin`, on `upload/{id}/commit` and on the
  multipart `?action=upload`; `fail` answers `409 EXISTS` before anything is
  written or snapshotted, and a refused commit keeps the session so it can be
  re-committed with `replace`. A target is locked with `409 UPLOAD_IN_PROGRESS`
  while another session is committing, or has taken a chunk within 30 s. "Keep
  both" is still the client's rename; "ask" is the client asking after `fail`.
