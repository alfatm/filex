...
<template>
  <article :aria-label="hit.node.name" class="flex items-start px-3 py-2">
    <HitIcon :node="hit.node" :size="24" />
    <div class="ml-2.5 min-w-0 flex-1">
      <p class="truncate-safe text-12 font-medium leading-tight">{{ hit.node.name }}</p>
      <p class="mt-0.5 truncate-safe text-11 leading-none text-text-3">{{ folderLabel }}</p>
      <p v-if="hit.snippet" class="mt-1 text-11 leading-tight text-text-3">
