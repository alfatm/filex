...
<template>
  <article :aria-label="hit.node.name" class="flex items-start rounded-lg border border-border p-[14px]">
    ...
    <div class="ml-3 min-w-0 flex-1">
      <p class="truncate-safe text-16 font-medium leading-none">{{ hit.node.name }}</p>
      <p class="mt-1 truncate-safe text-14 leading-none text-text-3">{{ folderLabel }}</p>
      <p v-if="hit.snippet" class="mt-1 text-14 leading-tight text-text-3">
